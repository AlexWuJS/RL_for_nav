...
from hierarchical_mppi_wrapper import HierarchicalMppiV4Wrapper, hierarchical_mppi_v4_config
...
class EntropyControlCallback(BaseCallback):
    ...
    def _on_step(self) -> bool:
        if self.num_timesteps >= self.switch_step and not self.switched:
            print(f"[hierarchical-v4] switching to fine-tune phase at {self.num_timesteps} steps")
...
def make_env(
    ...
    def _init():
        ...
        env = HierarchicalMppiV4Wrapper(
            env,
            config=hierarchical_mppi_v4_config(seed=seed + rank, reward_profile=reward_profile),
            reward_profile=reward_profile,
        )
        env = Monitor(env, filename=os.path.join(log_dir, f"hierarchical_v4_{reward_profile}_monitor_{rank}"))
        return env
...
def parse_args():
    parser = argparse.ArgumentParser(description="Train SAC semi-discrete v4 intent policy with trigger-based MPPI control.")
    ...
    parser.add_argument("--reward-profile", choices=["compat", "enhanced"], default="compat")
...
def main():
    ...
    save_dir = args.save_dir or f"./training_hierarchical_mppi_v4_{profile}_results/"
    log_dir = args.log_dir or f"./logs_hierarchical_mppi_v4_{profile}/"
    tensorboard_log = args.tensorboard_log or f"./sac_hierarchical_mppi_v4_{profile}_log/"
    ...
    checkpoint_callback = CheckpointCallback(
        ...
        save_path=save_dir,
        name_prefix=f"hierarchical_v4_{profile}_sac_checkpoint",
    )
    ...
    if args.load_model:
        ...
        print(f"Resuming hierarchical SAC-MPPI v4 ({profile}) from: {model_path}")
    ...
    print(f"Training hierarchical SAC-MPPI v4 ({profile}) with action space: {train_env.action_space}")
    ...
    model.save(os.path.join(save_dir, f"final_model_hierarchical_v4_{profile}"))
    train_env.close()
    eval_env.close()
