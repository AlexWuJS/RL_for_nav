...
from hierarchical_mppi_wrapper import HierarchicalMppiV3Wrapper, hierarchical_mppi_v3_config
...
class EntropyControlCallback(BaseCallback):
    ...
    def _on_step(self) -> bool:
        if self.num_timesteps >= self.switch_step and not self.switched:
            print(f"[hierarchical-v3] switching to fine-tune phase at {self.num_timesteps} steps")
...
def make_env(
    ...
    def _init():
        env = MyCarEnv()
        env = HierarchicalMppiV3Wrapper(
            env,
            config=hierarchical_mppi_v3_config(seed=seed + rank),
            intent_ema_alpha=intent_ema_alpha,
            intent_hold_steps=intent_hold_steps,
        )
        env = Monitor(env, filename=os.path.join(log_dir, f"hierarchical_v3_monitor_{rank}"))
        return env
...
def parse_args():
    parser = argparse.ArgumentParser(description="Train SAC long-horizon Frenet intent policy with MPPI control.")
    ...
    parser.add_argument("--save-dir", default="./training_hierarchical_mppi_v3_results/")
    parser.add_argument("--log-dir", default="./logs_hierarchical_mppi_v3/")
    parser.add_argument("--tensorboard-log", default="./sac_hierarchical_mppi_v3_log/")
...
def main():
    ...
    train_env = DummyVecEnv([
        make_env(0, args.log_dir, args.seed, args.intent_ema_alpha, args.intent_hold_steps)
    ])
    eval_env = DummyVecEnv([
        make_env(1, args.log_dir, args.seed + 10000, args.intent_ema_alpha, args.intent_hold_steps)
    ])
    ...
    checkpoint_callback = CheckpointCallback(
        ...
        name_prefix="hierarchical_v3_sac_checkpoint",
    )
    ...
    if args.load_model:
        ...
        print(f"Resuming hierarchical SAC-MPPI v3 from: {model_path}")
    ...
    print(f"Training hierarchical SAC-MPPI v3 with action space: {train_env.action_space}")
    ...
    model.save(os.path.join(args.save_dir, "final_model_hierarchical_v3"))
    train_env.close()
    eval_env.close()
