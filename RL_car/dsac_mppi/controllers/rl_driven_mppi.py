...
from dsac_mppi.controllers.mppi_dbas import MPPIDBaSConfig, MPPIDBaSOptimizer
...
class RLDrivenMPPIActionWrapper(gym.Wrapper):
    ...
    def __init__(
        ...
        self._obs_history = []
        self.high_level_control = getattr(env, "control_mode", "") == "high_level_frenet"
        if self.high_level_control:
            self.optimizer.config.use_rl_initialization = False
            self.optimizer.config.use_hss = False
            self.optimizer.config.use_terminal_q = False
            self.optimizer.config.strict_terminal_q = False
    ...
    def step(self, action):
        ...
        high_level_info = {}
        if policy_action.size >= 3 and hasattr(self.env, "high_level_action_to_low_level"):
            raw_action, high_level_info = self.env.high_level_action_to_low_level(policy_action)
            raw_action = np.asarray(raw_action, dtype=np.float32).reshape(-1)[:2]
            planner_state = dict(planner_state)
            planner_state.update(high_level_info)
            adapter_observation = None
        else:
            raw_action = policy_action[:2]
        ...
        info["policy_action"] = policy_action.astype(np.float32)
