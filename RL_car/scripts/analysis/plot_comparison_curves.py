...
MODE_COLORS = {
    "dsac": "#2f6fdd",
    "pure_mppi": "#7f7f7f",
    "dsac_rl_driven_mppi": "#005f73",
    "dsac_rl_driven_mppi_no_hss": "#94d2bd",
    "dsac_rl_driven_mppi_fixed_sigma": "#ee9b00",
    "dsac_rl_driven_mppi_no_q": "#ca6702",
}
...
def plot_action_source(result_dir: str, output_dir: str, max_steps: int) -> None:
    ...
    for ax, mode in zip(axes.flatten(), modes):
        ...
        series = {}
        sources = ("dsac", "mppi", "pure_mppi", "rl_driven_mppi", "dsac_rl_driven_mppi", "fallback")
        ...
        ax.stackplot(
            ...
            series["mppi"],
            series["pure_mppi"],
            series["rl_driven_mppi"],
            series["dsac_rl_driven_mppi"],
            series["fallback"],
            labels=["dsac", "mppi", "pure_mppi", "rl_driven_mppi", "dsac_rl_driven_mppi", "fallback"],
            colors=["#9ecae1", "#fb6a4a", "#7f7f7f", "#005f73", "#0a9396", "#74c476"],
        )
        ax.set_ylim(0, 1.0)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Plot comparison curves from DSAC+MPPI evaluation outputs.")
    parser.add_argument("--result-dir", default="./data/dsac_usv/eval/dsac_rlmppi_eval")
